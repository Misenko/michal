...
class Michal::Sources::OpenNebula < Michal::Sources::Base
  ...
  def initialize(name, logger)
    ...
    @client = Michal::DbClient.new logger
    @collection = client.read_one(:collections, name: name)['current']
  end
  ...
  def vms_for_user(username, from, to)
    client.distinct(collection, {'VM.UNAME' => username, 'VM.HISTORY_RECORDS.HISTORY.RSTIME' => {'$lte' => to}, '$or' => [{'VM.HISTORY_RECORDS.HISTORY.RETIME' => {'$gte' => from}}, {'VM.HISTORY_RECORDS.HISTORY.RETIME' => 0}]}, 'VM.DEPLOY_ID')
  end
  ...
  def vms_for_group(group_name, from, to)
    client.distinct(collection, {'VM.GNAME' => group_name, 'VM.HISTORY_RECORDS.HISTORY.RSTIME' => {'$lte' => to}, '$or' => [{'VM.HISTORY_RECORDS.HISTORY.RETIME' => {'$gte' => from}}, {'VM.HISTORY_RECORDS.HISTORY.RETIME' => 0}]}, 'VM.DEPLOY_ID')
  end
  ...
  def cpu_sum(vm_deploy_ids, time)
    ...
    client.aggregate(collection, match_operator, group_operator)
  end
